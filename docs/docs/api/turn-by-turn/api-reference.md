...
| **BETA** `low_speed_vehicle` | Street-legal low-speed vehicles for short-range, local travel (examples: golf carts and neighborhood electric vehicles).                                                                                                                                                                                                                                                                                                |
...
##### Low Speed Vehicle costing options -> **BETA**
Standard costing for travel by [low-speed vehicles](https://en.wikipedia.org/wiki/Low-speed_vehicle) such as golf carts and [neighborhood electric vehicles](https://en.wikipedia.org/wiki/Neighborhood_Electric_Vehicle).
Note that the vehicle type can change which roads are accessible, so read the description carefully.
While the default vehicle type for this costing model is `low_speed_vehicle`,
the `golf_cart` type has received significantly more attention and real-world testing.

| LSV options               | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|:--------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `top_speed`               | The vehicle's top speed. This impacts travel time calculations as well as which roads are preferred. A very low speed vehicle will tend to prefer lower speed roads even in the presence of other legal routes.                                                                                                                                                                                                                                                                            |
| `max_allowed_speed_limit` | The maximum speed limit for highways on which it is legal for the vehicle to travel. Defaults to 57 (kph; around 35 mph). Acceptable values range from 20 to 80. Highways with _tagged_ speed limits higher than this value will not be routed over.                                                                                                                                                                                                                                       |
| `vehicle_type`            | The type of vehicle. The default type is `low_speed_vehicle`. <ul><li>`low_speed_vehicle`: a low-speed vehicle which falls under a different regulatory and licensing regime than automobiles (ex: LSV in the US and Canada, Quadricycles in the EU, etc.). </li><li>`golf_cart`: a street legal golf cart that is under a similar regulator regime as the generic LSV laws, but may need to follow special paths when available or abide by restrictions specific to golf carts.</li><ul> |
